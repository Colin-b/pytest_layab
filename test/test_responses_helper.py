...
def test_add_get_response_with_pattern_url():
    responses_helper.add_get_response(re.compile("http://t.*t"), "test value")
    response = requests.get("http://test")
    assert response.status_code == 200
...
def test_received_json():
    ...
    assert responses_helper.received_json("http://test/post") == {
        "key": ["value 1", "value 2"]
    }
...
def test_received_json_headers():
    ...
    assert responses_helper.received_json("http://test/post", {"X-Test": "Test"}) == {
        "key": "value"
    }
...
def test_received_form_invalid_header_regex():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(temp_file_path, "rb") as file:
            requests.post(
                "http://test/post", data={"key": "value"}, files={"file1": file.read()}
            )
...
def test_received_text_as_bytes():
    responses_helper.add_post_response("http://test/post", {})
    requests.post(
        "http://test/post", b"this is 5 text", headers={"Content-Type": "text/plain"}
    )
...
def test_received_text_headers():
    ...
    assert (
        responses_helper.received_text("http://test/post", {"Content-Type": "text/csv"})
        == "this is text"
    )
...
def test_received_text_with_unexpected_headers():
    ...
    with pytest.raises(Exception):
        responses_helper.received_text(
            "http://test/post", {"Content-Type": "text/csv2"}
        )
